...
class Pages:
    ...
    def page_write(self):
        ...
        max_chars = 20 
        title = str(self.pgTitle)

        if len(title) <= max_chars:
            self.display.text(title, 6, 8, 1)
            value_y = 17
        else:
            self.display.text(title[:max_chars], 6, 0, 1)                  
            self.display.text(title[max_chars:], 6, 8, 1)                  
            value_y = 17 if len(title[max_chars:]) <= max_chars else 25   



        self.display.text(str(value) + self.unit, 6, value_y, 1)
...
class managerPages:
    ...
    def push(self, page):
        ...
        self.pageListProbe.append(page)
        self.size += 1
        return 1
